...
### Keepalive for free-tier hosting

If your host suspends inactive apps, this repo now includes a built-in keepalive option:

1. Push the repo to GitHub.
2. In the GitHub repo, open `Settings -> Secrets and variables -> Actions`.
3. Add a secret named `KEEPALIVE_URL`.
4. Set it to your deployed public URL, for example `https://your-app-name.onrender.com`.
5. Leave `.github/workflows/keepalive.yml` enabled.

That workflow sends one request every 12 hours, which is enough to prevent a 24-hour inactivity timeout and usually enough for shorter idle windows too.

If you prefer your own scheduler, use:

```bash
KEEPALIVE_URL=https://your-app-name.onrender.com python scripts/keepalive_ping.py
```

The helper script treats `2xx`, `3xx`, and `4xx` responses as success because the host still received a real request. Only `5xx` responses count as failure.
